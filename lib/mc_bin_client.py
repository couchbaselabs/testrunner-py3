...
class MemcachedClient(object):
    ...
    def set(self, key, exp, flags, val, collection=None):
        ...
        collection = self.collection_name(collection)
        # self._set_vbucket(key, vbucket = -1, collection=collection)
    ...
    def delWithMetaLWW(self, key, exp, flags,cas, collection=None):
        ...
        return self._doCmd(memcacheConstants.CMD_DELETE_WITH_META, key, '',
    ...
    def observe(self, key, vbucket=-1, collection=None):
        ...
        collection = self.collection_name(collection)
        self._set_vbucket(key, vbucket, collection=collection)
        value = struct.pack('>HH', self.vbucketId, len(key)) + to_bytes(key)
    ...
    def collection_name(self, collection):
        # self.hello(memcacheConstants.FEATURE_COLLECTIONS)
    ...
    def get_collections(self, update_map=True):
